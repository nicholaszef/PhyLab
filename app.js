...
// === SIMULASI JATUH BEBAS SEDERHANA ===
function startJatuhBebasSimulation() {
  ...
  const restitution = parseFloat(document.getElementById('e').value) || 0.8;
...
function startProyektilSimulation() {
  ...
  const v0 = parseFloat(document.getElementById('velocityProyektil').value);
  const angle = parseFloat(document.getElementById('angleProyektil').value) * (Math.PI / 180);
  const h0 = parseFloat(document.getElementById('heightProyektil').value);
  const g = 9.8;

  let ball = {
    x: 40,
    y: canvas.height - h0 - 20,
    r: 10,
    color: '#facc15',
  ...
  const resetBtn = document.getElementById('resetProyektil');
  if (resetBtn) {
    resetBtn.onclick = () => {
      time = 0;
      ball.x = 40;
      ball.y = canvas.height - h0 - 20;
      ball.vx = v0 * Math.cos(angle);
      ball.vy = -v0 * Math.sin(angle);
      paused = false;
      animate();
    };
  }
  ...
  function update() {
    if (paused) return;
    time += 0.016;
    ball.x = 40 + ball.vx * time;
    const yPos = h0 + ball.vy * time - 0.5 * g * time * time;
  ...
  function animate() {
    update();
    drawBall();
    if (ball.x < canvas.width && !paused) requestAnimationFrame(animate);
  }
...
function startPendulumSimulation() {
  ...
  const length = parseFloat(document.getElementById('lengthPendulum').value) * 100;
  const damping = parseFloat(document.getElementById('dampingPendulum').value);
  const mass = parseFloat(document.getElementById('massPendulum').value);
  const g = 9.8;
  ...
  function drawPendulum() {
    ...
    ctx.beginPath();
    ctx.arc(bobX, bobY, 20 * mass, 0, Math.PI * 2);
  ...
  function update() {
    if (paused) return;
    angularAcc = (-g / length) * Math.sin(angle);
    angularVel += angularAcc;
    angularVel *= (1 - damping);
    angle += angularVel;
  }
...
// guard untuk mencegah submit ganda saat request berjalan
let isSendingDiscussion = false;
...
async function addNewDiscussionToDB() {
  ...
  if (isSendingDiscussion) return; // sudah ada pengiriman berjalan
